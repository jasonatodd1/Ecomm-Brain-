...
Last updated: 2026-05-22 (Fri, twelfth refresh). Reflects state after **white-space triangulation scoring** — demand × supply-quality matrix on candidate opportunities.
...
- **Supabase** (Postgres + RLS). 13 tables in `public` (added `assets` via migration 0007):
  ...
  - `opportunities` (11) — scored signals; **9 candidates** enriched with white-space triangulation (`gap_classification`, `white_space_score`, `quadrant`, `gap_analysis` jsonb — migration 0009)
...
- **`score-whitespace.ts`** — `npm run score:whitespace`. White-space triangulation on candidate-pool opportunities (no listing, no brief): Etsy search per keyword → `computeCompetitiveLandscape` → `scoreWhitespace()` (demand × supply_weakness matrix) → persist on `opportunities` (migration 0009). Deterministic, no LLM. Reddit rows search by post title; Trends rows search by `name`. See `COMPETITIVE_SEO_SCORING.md` §4 triangulation + `src/lib/whitespace-scoring.ts`.
...
| Scoring | **Built, MVP validated; white-space triangulation live** | Demand-only scoring in `score-opportunities.ts`; supply-side gap + triangulation in `score-whitespace.ts` (migration 0009). Smoke-tested on 9 candidate-pool opps. Next: broad Trending Now collector to widen the field, then re-run white-space scoring. |
