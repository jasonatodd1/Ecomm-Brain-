...
Last updated: 2026-05-26. Reflects **broad Trending Now collector** + first broad white-space run.
...
- **`competitive.ts`** — supply-side gap discovery (Tuning Pass 2). `computeCompetitiveLandscape({ keywords, resultsByKeyword, topN=10 })` … emits one `CompetitiveLandscapeEntry` per keyword carrying top-3 incumbents (now with `num_favorers`, `views`, `shop_review_count`) + scores + weak_areas + medians (`median_favorers`, `median_views`, `median_shop_reviews`) + synthesis-ready `gap_summary`. Also fetches shop review counts (deduped, same concurrency posture) for engagement proxy. Consumed by Research Agent **and** standalone white-space job.
