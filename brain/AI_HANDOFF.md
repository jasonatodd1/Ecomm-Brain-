...
- **`run-niche-bakeoff.ts`** — `npm run bakeoff [--run-label=…] [--treatment=baseline|pinterest]`. Curated 9-niche keyword bake-off: composable demand (`bakeoff-demand.ts`: Google now + Etsy incumbent engagement; Pinterest slot null in baseline) → shared `gapScoreKeyword` → `whitespace-scoring.ts`. Persists to `niche_bakeoff_runs` / `niche_bakeoff_results` (migration 0010) — **does not** touch `opportunities`. Reports RAW ranking, BY-NICHE summary (digital printables = ANCHOR), and DIGITAL-PREFERRED view (physical/dropship must beat best digital by ≥0.10 WS). Keyword set + coherent-phrase rule in `bakeoff-keywords.ts` + `NICHE_BAKEOFF.md`.
...
- **`bakeoff-demand.ts`** — composable external demand for niche bake-off. Baseline: `external = google_demand`; treatment slot: `0.45×google + 0.55×pinterest` when `pinterest_demand` populated.
...
| Scoring | **Built; Pass A/B/C + white-space triangulation + niche bake-off** | Seeds + Reddit + Trending Now → opportunities; `score:whitespace --limit=40` gap-scores top-N by demand. **Niche validation bake-off:** `npm run bakeoff` — baseline (Etsy+Google, no Pinterest) vs future Pinterest treatment; neutral `white_space_score` + separate digital decision-hurdle (≥0.10 margin); coherent-keyword rule (specific buyer phrases only). Baseline run 2026-05-26: SVG/craft digital beats anchor digital printables on raw WS; anchor #2 by niche. |
