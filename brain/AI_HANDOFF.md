...
Last updated: 2026-05-21 (Thu, eighth refresh — late evening). Reflects state after the asset registry commit (migration 0007 + `link:asset` CLI + write-into-assets integration across all 4 asset-producing tools). The `assets` table is populated for both live listings (12 rows backfilled, 0 missing-disk paths) and every future `gen` / `upscale` / `build:bundle` / `resize:print` run will auto-register its outputs. **All Listing Agent prerequisites are now cleared** — Tuning Pass 2, the SEO scoring engine, the full asset pipeline, AND the asset registry are all in. The Listing Agent itself is the only remaining work item for that build.
...
| Listing Agent | **Not built — spec'd; all prereqs cleared** | Full requirements in `brain/LISTING_AGENT_REQUIREMENTS.md` (10 sections + worked examples from manual publishes). Store-agnostic core + per-store adapters (Etsy → Pinterest → Shopify). **All upstream blockers now cleared:** (1) Research Agent v2 schema (Tuning Pass 2), (2) shared `scoreEtsyListingSeo()` engine, (3) all 3 data-quality bugs (niche/source_count/Reddit post_url), (4) full asset pipeline (`npm run build:bundle` produces every v2-brief-promised deliverable), AND (5) **the asset registry** (migration 0007 + `link:asset` CLI + write-into-assets across all 4 producers — populated for both live listings, 12 rows). The agent itself is the next concrete build. The one remaining design question (live store-schema fetch + caching so attribute values are verified against `possible_values` at runtime, per spec §3/§4) is part of the Etsy adapter implementation, not a separate prereq. Etsy publish itself is currently fully manual. |
