...
Last updated: 2026-05-22 (Fri, eleventh refresh). Reflects state after **bunny size-guide listing photo** (`feat: bunny size-guide listing photo`) — bunny Listing Agent image manifest **5/5 ready** (all brief.image_spec slots covered). Description reconciliation analysis done (brief wins; manual Etsy paste pending with deliverables fix).
...
- **`attribute-mapping.ts`** — codification of `LISTING_AGENT_REQUIREMENTS.md` §4 (allowed-value verification). `mapSemanticToAllowed({ descriptors, property })` → `{ value, value_id, was_substituted, reason, matched_from, confidence }` or null (single-valued properties). `mapSemanticToAllowedMany` → `{ matches[], skipped[] }` (multi-valued: Etsy Occasion, Material multi, Art subject, Holiday). Three tiers with explicit fail-to-blank: tier 1 exact normalized equality → tier 2 whole-token containment (descriptor is a token of the value, or vice versa) → tier 3 curated `SEMANTIC_SUBSTITUTIONS` map (cottagecore→country & farmhouse, scandinavian→minimalist, "digital download"→Paper, sage green→Green, etc.) → null. Never force-picks; null means the slot stays blank per §4 ("blank slot is always better than a wrong slot"). Pure / deterministic / no LLM in v1 — the substitution map is small, explicit, and reviewable.
...
- `brain/supabase/migrations/` — `0001_init.sql`, `0002_rls.sql`, `0003_opportunities_unique_name.sql`, `0005_foundation_schemas.sql`, `0006_listings_monitoring.sql`, `0007_assets.sql` (0004 absent)
...
| Scoring | **Built, MVP validated; all 3 data-quality bugs fixed** | Bug fixes shipped in the `fix(data-quality)` commit: `opportunities.niche` now written on every upsert (Reddit→subreddit, Trends→'general'), `source_count` aggregated across the in-memory signal pool (Trends count by keyword, Reddit count by `post_id`), Reddit signal metadata renamed `url`→`post_url` for cross-table consistency. Scoring formula ceiling (multiple keywords hitting confidence 1.000) remains a separate backlog refactor in §7. |
