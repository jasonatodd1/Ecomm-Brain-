...
Last updated: 2026-05-21 (Thu, seventh refresh — evening). Reflects state after the asset-pipeline expansion commit. `npm run build:bundle` now produces the full deliverable set every v2 wall-art brief promises (master JPG + sized JPG set + multi-page print-bundle PDF with crop marks + transparent PNG via fal birefnet/v2 + single-page ratio-guide PDF) — smoke-tested on the bunny in 41.7s at $0.00 fal cost. Listing Agent prerequisites narrowed from four down to one (the `assets` table + `link:asset` CLI for backfilling pre-existing assets); the brief-promises-vs-pipeline-produces gap is now closed.
...
### `src/tools/`
- **`resize-print-variants.ts`** — `npm run resize:print <input.png> [output-dir]`. Legacy single-purpose CLI from before the full bundle pipeline shipped — produces only the 5 sized JPG variants (`HillwardStudio-<NamePrefix>-{8x10,11x14,16x20,18x24,24x36}.jpg`). Preserved as a working entry point; delegates all crop coords / sharp ops / VARIANT definitions to the shared `src/lib/print-bundle.ts`. For the complete brief-promised deliverable bundle (master JPG + sized JPGs + PDFs + transparent PNG), use `build-print-bundle.ts` instead.
- **`build-print-bundle.ts`** — `npm run build:bundle -- --master=<path> --product=<slug>`. Full deliverable orchestrator. Reads a 5008×6680 master and produces every artifact a v2 wall-art brief promises a buyer: `master.jpg` (300 DPI) + `sized/<prefix>-*.jpg` (5 print sizes) + `print-bundle.pdf` (multi-page, one page per size, with crop marks at every trim corner and per-page footer label) + `transparent.png` (background removed via fal birefnet/v2 — preserves full input resolution thanks to `refine_foreground=true`) + `ratio-guide.pdf` (single page US Letter, legend + scale comparison diagram + frame recommendations). Outputs land at `products/<slug>/deliverables/` (gitignored). One `activity` row per deliverable (`agent='product', action='asset.built'`) carrying kind / path / size / duration / cost / per-kind metadata so the future Listing Agent can verify brief claims against on-disk assets without filesystem inspection. Dual-mode: CLI or `import { buildPrintBundle }`. Smoke-tested on the bunny 2026-05-21: 41.7s end-to-end, 140.2 MB total, $0.00 fal cost (birefnet is free).
...
| Listing Agent | **Not built — spec'd; one prereq left** | Full requirements in `brain/LISTING_AGENT_REQUIREMENTS.md` (10 sections + worked examples from manual publishes). Store-agnostic core + per-store adapters (Etsy → Pinterest → Shopify). Remaining prerequisites: (1) `assets` table + `link:asset` CLI to backfill bunny + planner assets generated outside the system (§6), (2) live store-schema fetch + caching so attribute values are verified against `possible_values` at runtime (§3, §4). **Cleared today:** Research Agent v2 schema (Tuning Pass 2), the shared `scoreEtsyListingSeo()` engine, all 3 data-quality bugs (niche/source_count/Reddit post_url), AND the asset-pipeline gap — `npm run build:bundle` now produces the full v2-brief-promised deliverable set, so the agent has nothing to discover or backfill on the deliverables side when it publishes. Etsy publish itself is currently fully manual. |
...
- _Asset pipeline gap DONE — `npm run build:bundle` now produces the full deliverable set every v2 brief promises._
...
Tuning Pass 2 + data-quality bug fixes + asset-pipeline expansion are all in. The pipeline is end-to-end clean: discovery → scoring → research → brief → render → asset bundle. The Listing Agent is the next major work item with only one upstream blocker remaining (the `assets` table + `link:asset` CLI for backfilling pre-existing manual assets).

1. **Begin Listing Agent build.** All other blockers cleared. Concrete first slice per `LISTING_AGENT_REQUIREMENTS.md`: (a) migration `0007_assets.sql` defining the `assets` table (kind, listing_id, product_brief_id, dims, source, cdn_url, fal_request_id, deliverable_kind); (b) extend `src/tools/generate-image.ts`, `src/tools/upscale-image.ts`, AND `src/tools/build-print-bundle.ts` to UPSERT into `assets` in addition to the existing `activity` log — the bundle orchestrator already logs per-deliverable, so the upsert just adds DB rows with the same metadata; (c) ship `npm run link:asset` CLI to backfill the bunny + planner manual assets generated outside the system before this infra existed; (d) then the agent skeleton itself — store-agnostic core + Etsy adapter, consuming the v2 brief + the SEO scorer as a pre-publish quality gate and walking `brief.product.format.deliverables[]` to verify every claimed file exists on disk. Etsy first; Pinterest second; Shopify deferred.
