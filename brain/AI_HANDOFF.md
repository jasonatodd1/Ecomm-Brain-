...
Last updated: 2026-05-21 (Thu, sixth refresh — evening). Reflects state after the data-quality fix commit. All three previously-known data-quality bugs are closed: `opportunities.niche` populated (0 → 11), `opportunities.source_count` aggregating across signals (0 → 9 with sc > 1), Reddit `signals.metadata.post_url` populated (0 → 16). Combined with Tuning Pass 2 shipping earlier today, the discovery → scoring → research → brief pipeline is now end-to-end trustworthy with zero known data bugs. Listing Agent prerequisites narrowed from four to one (asset registry + `link:asset` CLI).
...
| Listing Agent | **Not built — spec'd; ready to start** | Full requirements in `brain/LISTING_AGENT_REQUIREMENTS.md` (10 sections + worked examples from manual publishes). Store-agnostic core + per-store adapters (Etsy → Pinterest → Shopify). Remaining prerequisites: (1) `assets` table + `link:asset` CLI to backfill bunny + planner assets generated outside the system (§6), (2) live store-schema fetch + caching so attribute values are verified against `possible_values` at runtime (§3, §4 — direct response to the lesson that Etsy attribute schemas vary by taxonomy). **Cleared in earlier commits today:** Research Agent v2 schema (audience.persona, structured `listing.description`, semantic `attribute_intent`, `image_spec[]`, `shop_section_suggestion`, `competitive_landscape`), the shared `scoreEtsyListingSeo()` engine (`brain/src/lib/etsy-seo-scoring.ts`), AND all three data-quality bugs that would have fed garbage into a Listing Agent (niche null, source_count stuck at 1, Reddit post_url missing). Per Backlog: also pending an asset-pipeline decision (expand `resize-print-variants.ts` to produce master JPG + transparent PNG + crop-marked PDF + ratio guide, OR constrain the brief schema so the agent only claims files the pipeline produces today). Etsy publish itself is currently fully manual. |
...
Tuning Pass 2 + data-quality bug fixes are both in. The pipeline is end-to-end clean: discovery → scoring (now with correct `niche`, aggregating `source_count`, and consistent Reddit `post_url`) → research → brief (v2 schema with structured `listing.description` + competitive landscape) → render (markdown + Etsy plaintext). The Listing Agent is the next major work item.

1. **Begin Listing Agent build.** All upstream blockers cleared. Concrete first slice per `LISTING_AGENT_REQUIREMENTS.md`: (a) migration `0007_assets.sql` defining the `assets` table (kind, listing_id, product_brief_id, dims, source, cdn_url, fal_request_id); (b) extend `src/tools/generate-image.ts` and `src/tools/upscale-image.ts` to UPSERT into `assets` in addition to the existing `activity` log; (c) ship `npm run link:asset` CLI to backfill the bunny + planner assets generated outside the system before this infra existed; (d) decide the asset-pipeline gap (expand `resize-print-variants.ts` to produce the full deliverable set the v2 bunny brief claims, OR constrain the brief's `whats_included` to only files the pipeline produces today). Then the agent skeleton itself — store-agnostic core + Etsy adapter, consuming the v2 brief + the SEO scorer as a pre-publish quality gate. Etsy first; Pinterest second; Shopify deferred.
...
Secondary, if time allows: capture more competitive-landscape evidence by re-running `research-decision` on any new opportunities surfaced during the next scoring run (now that scoring writes correct `niche` + aggregated `source_count`, the queue should look more useful). Also worth noting: with all data-quality bugs cleared, the next data-quality issue to surface will be the scoring-formula ceiling (multiple Trends keywords saturating at confidence 1.000) — backlog refactor, but it's the next discrimination wall.
