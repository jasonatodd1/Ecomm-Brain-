...
- [ ] **Paid keyword-volume tool** (eRank / Marmalead / SerpApi-equivalent) for real search-volume data, per principle #6 (paid APIs over fragile scraping). **Also closes the Pinterest demand slot:** the only viable paths to real Pinterest volume are a gated partner API or a paid SEO tool — Pinterest-as-discovery is folded into this item rather than tracked as its own task. (Pinterest-as-distribution is the separate Listing Agent adapter above.)
