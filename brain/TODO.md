...
### Visual Competitive Analyst (built + validated 2026-06-02)
- ✅ **Visual analyst built and validated on wedding day timeline.** `npm run visual:analyze -- --decision-id=<uuid>` — fetches competitor listing images (Etsy `/listings/{id}/images`) + API `has_video` (`/listings/{id}/videos`), analyzes with Claude Vision (`claude-sonnet-4-6`), and writes a cross-listing synthesis to `niche_memory` (`key=visual_competitive_intel`, `source=visual_analyst`, `confidence=0.85`) + a markdown report at `brain/briefs/visual-{date}-{niche_tag}.md`. `niche_tag` is set explicitly by slugifying the decision keyword (not the subreddit resolver). Validated on `60dbd2ae`: 13 listings, 38 images, **46% API-confirmed video presence**, $0.26, 108s. Runs independently of the Research Agent.
- [ ] **Next: incorporate `visual_competitive_intel` into the Research Agent brief synthesis prompt** so future briefs automatically include the photo set + video spec (load the `visual_competitive_intel` niche_memory row at synthesis and fold `recommended_photo_set` / `recommended_video_concept` / `visual_gaps` into the brief's image spec).
- [ ] **Then: run the visual analyst on each new opportunity before product build** (make it a standard pre-build step alongside the Research Agent brief).
...
- [ ] **Demand stability score (consolidated).** Single-window Google Trends velocity is volatile — keywords swing +8%→+494% in one cycle, and even bake-off keywords whose head==specific swung run-to-run (e.g. `funny mom shirt` ext 1.0→0.0, `custom name t shirt` 0.71→0.0 across the v2→v3-phrasefix dates). Need historical velocity tracking + a stability score (re-pull or longer-window confirmation for borderline niches) before trusting single-run velocity for bake-off / whitespace ranking. _(The orthogonal bake-off phrase-sensitivity fix — head-term demand / specific-phrase supply — shipped 2026-05-28; see CHANGELOG. That fixed long-tail false-negatives; this item is the remaining run-to-run volatility piece.)_
